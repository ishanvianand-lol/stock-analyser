...
def get_data(stock_name):
    ...
    # 1️⃣ Create figure and axis
    fig, ax = plt.subplots(figsize=(14,7), facecolor="#FFFFFFC2")  # wide figure for readability
    ax.set_facecolor("#FFFFFF94")  # subtle background

    # 2️⃣ Plot actual closing price
    ax.plot(data['Date'], data['Close'], label='Close', color='black', linewidth=3)

    # 3️⃣ Plot moving averages
    ...
    # 4️⃣ Highlight bullish/bearish zones (MA5 vs MA20)
    ax.fill_between(
    ...
    # 5️⃣ Add title, labels, and legend
    ax.set_title(f'({stock_name})'.upper(), fontsize=18, fontweight='bold')
    ax.set_xlabel('Date', fontsize=14)
    ax.set_ylabel('Price', fontsize=14)
    ax.legend()

    # 6️⃣ Rotate x-axis dates for readability
    plt.xticks(rotation=20)

    # 7️⃣ Add grid for easier trend reading
